...
public class ClassLogServiceImpl implements ClassLogService {
  ...
  public ClassLog[] recommendClassByStuId(int sId) {
    ...
    // 如果该用户为新用户，还没有认知信息
    if (stuCognitions.length == 0) {
      // 按课程资源评分排名来返回推荐课程
      ClassLog[] classLogs = classLogService.selectByScoreOrderLimNum(9);
      return classLogs;
    }
    ...
    // 看推荐的课程数量，最小应为9，如果不够九个，要加上高评分课程
    if (size < 9) {
      ClassLog[] tmpClassLogs = classLogService.selectByScoreOrderLimNum(9 - size);
      for (ClassLog tmpClassLog : tmpClassLogs) {
        resCIdList.add(tmpClassLog.getcId());
      }
    }
